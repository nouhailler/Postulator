...
Classe abstraite pour tous les scrapers Postulator.
Chaque scraper (JobSpy, Playwright custom…) hérite de BaseScraper.
"""
import asyncio
import random
...
class BaseScraper(ABC):
    ...
    Chaque sous-classe implémente `_fetch()` qui retourne une liste de RawJob.
    `run()` gère les délais anti-blocage, la rotation de proxies et les retries.
    """

    #: Nom court du scraper, utilisé dans les logs et ScrapeLog
    ...
    async def run(
        ...
        Ajoute délai aléatoire + gestion d'erreurs autour de _fetch().
        """
